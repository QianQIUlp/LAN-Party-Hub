...
import { zhAdultWordPool, zhStandardWordPool } from "./zhWordPools.js";

const roundDurationMs = 90_000;
const maxGuessFeedEntries = 10;
const maxGuessLength = 40;
const maxStrokes = 240;
const maxDrawingPoints = 6_000;
const minimumPointDistance = 0.002;

const zeichnenUndErratenText = {
  "zh-CN": {
    drawerFallback: "画手",
    playerFallback: "玩家",
    ready: (name: string) => `${name} 即将作画，准备猜词！`,
    drawingNow: (name: string) => `${name} 正在作画。`,
    cleared: (name: string) => `${name} 清空了画板。`,
    guessedCorrectly: (name: string, word: string) => `${name} 猜中了“${word}”！`,
    guessed: (name: string, guess: string) => `${name} 猜：${guess}`,
    timeUp: (word: string) => `时间到，答案是“${word}”。`
  },
  de: {
...
const zhAllWordPool = [...zhStandardWordPool, ...zhAdultWordPool] as const;

const zhWordPoolByCategory = {
  standard: zhStandardWordPool,
  adult: zhAdultWordPool,
  all: zhAllWordPool
} as const satisfies Record<ZeichnenUndErratenWordCategory, readonly string[]>;
...
export function normalizeGuess(value: string, language: SupportedLanguage = "zh-CN"): string {
  return value
    .trim()
    .normalize("NFKC")
...
function countDrawingPoints(strokes: ZeichnenUndErratenState["strokes"]): number {
...
function pickWord(context: ServerGameContext, category: ZeichnenUndErratenWordCategory): string {
  const pools = context.language === "zh-CN" ? zhWordPoolByCategory : wordPoolByCategory;
  const pool = pools[category] ?? pools.standard;
  ...
  const fallbackWord = context.language === "zh-CN" ? "太阳" : "Sonne";
  const selected = pool[index] ?? pool[0] ?? fallbackWord;

  if (selected === previousWord && pool.length > 1) {
    return pool[(index + 1) % pool.length] ?? fallbackWord;
  }
  ...
  handleInput(state, input, context) {
    ...
    if (input.playerId === state.drawerPlayerId) {
      ...
      if (input.type === "draw:start") {
        if (!isFinitePoint(input.x, input.y) || state.strokes.length >= maxStrokes || countDrawingPoints(state.strokes) >= maxDrawingPoints) {
          return state;
        }
      ...
      if (input.type === "draw:move") {
        if (!state.activeStrokeId || state.strokes.length === 0 || !isFinitePoint(input.x, input.y) || countDrawingPoints(state.strokes) >= maxDrawingPoints) {
          return state;
        }
        ...
        if (previousPoint && Math.hypot(point.x - previousPoint.x, point.y - previousPoint.y) < minimumPointDistance) {
          return state;
        }
    ...
    const guess = input.guess.trim().slice(0, maxGuessLength);
    ...
    const isCorrect = normalizeGuess(guess, context.language) === normalizeGuess(state.secretWord, context.language);
