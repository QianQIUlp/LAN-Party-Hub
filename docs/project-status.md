...
- 新克隆包含八款固定源码版本的内置游戏：疯狂点击、你画我猜、估个大概、谁是卧底、吹牛牌、命运轮盘、谎言牌桌、迷雾仓库。
...
- 主屏大厅采用电视距离优先的编辑风分栏：房间码、二维码和玩家状态集中在深色侧栏，游戏目录使用高对比浅色网格，并在窄视口自动切换为双列或单列滚动布局。
- 双语静态项目介绍站可由 Cloudflare Pages 从 `main` 分支单独构建；它不加载 Host、Controller、Socket.IO 或游戏服务端，只提供项目说明、发行版和源码入口。
- Vitest 覆盖平台恢复和八款游戏的关键规则；Playwright 覆盖阻断公网后的双手机初始开局、控制器刷新恢复、双人命运轮盘、三人切换兼容游戏，以及人数限制。
...
八款游戏均为 alpha，仍需要真人聚会测试来调整节奏、难度、题库质量、命运轮盘时长和移动端误触；吹牛牌还需要重点验证极端长手牌时的重叠密度和大人数主屏可读性，迷雾仓库需要重点验证横屏信息密度和五轮估值节奏。

## 自动化验证

- `npm run test`：纯逻辑与 Socket.IO 重启恢复测试。
- `npm run test:e2e`：Chromium 独立上下文模拟主屏和两台手机，并阻断公网请求。
- `npm run typecheck`：平台及八款内置游戏类型检查。
