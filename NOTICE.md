...
Four bundled games originate from Apache-2.0 upstream repositories. Their exact repositories, fixed revisions, retained licenses, and modification scope are recorded in [THIRD_PARTY_SOURCES.md](THIRD_PARTY_SOURCES.md).

`bullshit`, `roulette`, and `liars-table` are original LAN Party Hub additions maintained in this repository under Apache-2.0. The `bullshit` generated media is documented in its asset ledger; `roulette` and `liars-table` use code-drawn interfaces and system text glyphs. None of the three imports third-party character art, card art, audio, fonts, or game text.
