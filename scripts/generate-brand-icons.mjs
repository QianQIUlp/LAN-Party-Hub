...
try {
  ...
  for (const size of icoSizes) {
    images.push({ size, png: await renderIcon(browser, size) });
  }

  const appIcon = await renderIcon(browser, 512);
  const favicon = images.find(({ size }) => size === 64).png;
  const ico = buildIconFile(images);
  const outputs = [
    [path.join(brandingRoot, "lan-party-hub.png"), appIcon],
    [path.join(brandingRoot, "lan-party-hub.ico"), ico],
